...
teal every frame, closing when we hit escape. It is not much to look at yet, but
it means GLFW, the OpenGL context, and the ~gl~ function pointers are all wired
up correctly. That is the whole point of this chapter: a known-good foundation
to build the actual rendering on top of.
